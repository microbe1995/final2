...
export default function LciaPage() {
  ...
  const handleStartRun = async () => {
    ...
    try {
      const result = await startLciaRun('current-project', config);
      if (result.success) {
  ...
  const fetchResults = async () => {
    try {
      const result = await getLciaResults('current-project', runId);
  ...
  const fetchHistory = async () => {
    try {
      const result = await getLciaHistory('current-project');
      if (result.success && result.data) {
