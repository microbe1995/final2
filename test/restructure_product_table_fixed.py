...
async def restructure_product_table():
    ...
    try:
        ...
        # 2단계: 의존 관계 해제 (외래키 제약조건 삭제)
        logger.info("🔗 2단계: 의존 관계 해제")
        for dep in dependencies:
        ...
        # 3단계: 기존 product 테이블 삭제
        logger.info("🗑️ 3단계: 기존 product 테이블 삭제")
        await conn.execute("DROP TABLE IF EXISTS product CASCADE")
        logger.info("✅ 기존 테이블 삭제 완료")
        
        # 4단계: product_core 테이블 생성
        logger.info("🏗️ 4단계: product_core 테이블 생성")
        await conn.execute("""
        ...
        # 5단계: 기존 데이터 복원 (핵심 정보만)
        logger.info("📊 5단계: 기존 데이터 복원")
        await conn.execute("""
            INSERT INTO product (
