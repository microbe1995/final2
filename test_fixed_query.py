...
async def test_fixed_query():
    ...
    try:
        ...
        AND (
            ...
            OR (투입일 BETWEEN $2::DATE AND $1::DATE)    -- 투입일이 기간 내에 있는 경우
            OR (종료일 BETWEEN $2::DATE AND $1::DATE)    -- 종료일이 기간 내에 있는 경우
            OR (투입일 <= $2::DATE AND 종료일 >= $1::DATE)  -- 기간을 완전히 포함하는 경우
        )
        ORDER BY 생산품명;
        """
        
        rows = await conn.fetch(new_query, END_DATE, START_DATE)
